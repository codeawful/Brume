/*
  Compact UI/controller for BRUME Field Lab.
...
class EditorUI {
  ...
  void drawTop() {
    ...
    fill(app.themeText); textAlign(LEFT,CENTER); textSize(13); text("BRUME / FIELD LAB",16,20);
  ...
  void keyPressed(char k,int code) {
    ...
    else if (k=='p'||k=='P') selectOutput("Save BRUME preset","presetOutputSelected");
    else if (k=='l'||k=='L') selectInput("Open BRUME preset","presetFileSelected");
  ...
  void action(String id) {
    ...
    else if (id.equals("preset.save")) selectOutput("Save BRUME preset","presetOutputSelected");
    else if (id.equals("preset.load")) selectInput("Open BRUME preset","presetFileSelected");
