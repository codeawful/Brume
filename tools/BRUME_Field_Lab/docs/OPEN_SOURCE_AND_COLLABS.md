...
> Originally built as an internal instrument while designing the BRUME identity. Released so other designers can subject their own letterforms to the same kind of controlled visual physics.
...
5. BRUME applies it to the canonical B.
...
Long term, separate the generic engine from a BRUME skin/example.
